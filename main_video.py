...
1. Uses dataloader_video.py instead of dataloader.py
2. Reports video-level statistics for train/val/test splits
3. Designed for video-level few-shot learning experiments
"""
...
# CHANGED: Import from dataloader_video instead of dataloader
from dataloader.dataloader_video import data_generator
...
# Logging
# Use microseconds to ensure unique filenames even for rapid successive runs
timestamp = datetime.now().strftime('%d_%m_%Y_%H_%M_%S_%f')
log_file_name = os.path.join(experiment_log_dir, f"logs_{timestamp}.log")
...
if training_mode == "gen_pseudo_labels":
    # CHANGED: Use 1shot instead of 1p for consistency
    ft_perc = "1shot"
    load_from = os.path.join(
...
if training_mode == "SupCon":
    # CHANGED: Use 1shot instead of 1p
    data_perc = "1shot"
    load_from = os.path.join(
        os.path.join(logs_save_dir, experiment_description, run_description, f"ft_{data_perc}_seed_{SEED}", "saved_models"))
    chkpoint = torch.load(os.path.join(load_from, "ckp_last.pt"), map_location=device)
    pretrained_dict = chkpoint["model_state_dict"]
    model.load_state_dict(pretrained_dict)
